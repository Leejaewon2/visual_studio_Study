...
-- SELECT 절에 사용하는 서브쿼리 : 스칼라 서브쿼리라고도 부름 (반드시 하나의 결과만 나와야 함)
-- 단일행으로 와야 함 
SELECT EMPNO, ENAME, JOB, SAL, (SELECT GRADE 
                                FROM SALGRADE
                                WHERE E.SAL BETWEEN LOSAL AND HISAL) AS 급여동급,
                                DEPTNO,
                                (SELECT DNAME
                                FROM DEPT
...
AND E.JOB = (SELECT JOB
            FROM EMP
            WHERE ENAME = 'ALLEN')
-- 2. 전체 사원의 평균 급여(SAL)보다 높은 급여를 받는 사원들의 사원 정보, 부서 정보, 급여 등급 
--    정보를 출력하는 SQL문을 작성하세요(단 출력할 때 급여가 많은 순으로 정렬하되 급여가 같을 경우에는 
--    사원 번호를 기준으로 오름차순으로 정렬하세요).
-- EMP, DEPT, SALGRADE
SELECT E.JOB, E.EMPNO, E.ENAME, E.SAL, E.DEPTNO, D.DNAME, D.LOC, G.GRADE
FROM EMP E JOIN DEPT D
ON E.DEPTNO = D.DEPTNO
...
-- 4. 직책이 SALESMAN인 사람들의 최고 급여보다 높은 급여를 받는 사원들의 사원 정보, 급여 등급 정보를 다음과 
--    같이 출력하는 SQL문을 작성하세요(단 서브쿼리를 활용할 때 다중행 함수를 사용하는 방법과 사용하지 않는 방법을
...
INSERT INTO DEPT_TEMP (DEPTNO, DNAME)(80, '백엔드');
...
-- 테이블에 날짜 데이터 입력하기
INSERT INTO EMP_TEMP(EMPNO, JOB, MGR, HIRDATE, SLA, COMM, DEPTNO)
