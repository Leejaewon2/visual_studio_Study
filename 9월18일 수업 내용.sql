-- GROUP BY : 여러 데이터에서 의미 있는 하나의 결과를 특정 열 값별로 묶어서
...
-- 부서코드, 급여, 합계, 부서 코드 순 정렬을
SELECT DEPTNO 부서코드, -- 이름 적어줄때 DEPTNO AS 부서코드 이렇게 써도되고 AS를 안붙이고 써도 됨.
    SUM(SAL) 합계,
    ROUND(AVG(SAL), 2) 평균,
...
-- GROUP BY 절을 통해 그룹화된 결과 값의 번위를 제한할 때 사용자
...
-- 1. 부서별 직책의 평균 급여가 500 이상인 사원들의 부서 번호, 직책의 평균 급여 출력
SELECT DEPTNO, JOB, ROUND(AVG(SAL), 2)
FROM EMP
GROUP BY DEPTNO, JOB
    HAVING AVG(SAL) >= 500
...
-- 5. 추가 수당을 받는사원 수와 받지 않는 사원수를 출력(O, X 로 표기 필요)
SELECT NVL2(COMM, 'O', 'X') AS "추가 수당",
    COUNT(*) AS 사원수
FROM EMP
GROUP BY NVL2(COMM, 'O', 'X');

-- 6. 각 부서의 입사 연도별 사원 수 , 최고급여, 급여 합, 평균 급여를 출력
SELECT DEPTNO, TO_CHAR(HIREDATE, 'YYYY') AS "입사 연도",
    TRUNC(AVG(SAL)) AS "평균 급여",
    SUM(SAL) AS "급여 합",
    MAX(SAL) AS "평균 급여",
    COUNT(*) AS 사원수
...
-- 급여가 2500이하이고 사원번호가 9999 이하인 사원의 정보가 출력 되도록 작성 (ANSI, 오라클 조인)
-- 오라클 조인
...
-- 외부 조인은 내부조인과 다르게 다른 한쪽에 값이 없어도 출력 됨
-- 외부 조인은 동등 조인 조건을 만족하지 못해 누락되는 행을 출력하기 위해 사용

INSERT INTO EMP(EMPNO, ENAME, JOB, MGR, HIREDATE,SAL)
    VALUES(9000, '장원영', 'SALEMANE', 7698, SYSDATE, 2000, 1000, NULL);
...
FROM EMP E JOIN DEPT D USING(D.DEPTNO);
...
-- 3. 모든 부서 정보와 사원 정보를 부서번호, 사원이름 순으로 정렬
SELECT E.*, D.DNAME AS 부서이름
...
-- EXISTS 연산자 : 서브쿼리의 결과 값이 하나 이상 존재 하면 조건식이 모두 TRUE, 존재하지 않으면 모두 FALSE
SELECT *
FROM EMP
